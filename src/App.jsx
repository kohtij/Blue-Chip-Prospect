...
// Accent + archetype styling maps for the data-driven minigame screen.
// Written as full literal class strings (not interpolated) so Tailwind's
// JIT can see them.
const ACCENT = {
  red:     { border: 'border-t-red-500',     heading: 'text-red-600' },
  blue:    { border: 'border-t-blue-500',    heading: 'text-blue-600' },
  emerald: { border: 'border-t-emerald-500', heading: 'text-emerald-600' },
  amber:   { border: 'border-t-amber-400',   heading: 'text-amber-500' },
};

const ARCH_PILL = {
  safe:   { label: 'SAFE',   cls: 'text-slate-600 bg-slate-50 border-slate-200',       hover: 'hover:border-slate-400' },
...
// (e.g. OHL/WHL Ottawa, WHL Seattle/Edmonton/Vancouver/Calgary) rendered
// the NHL crest instead of the junior team's colors. Now it also requires
// league === 'NHL'.
...
// Dashboard: BUG FIX #9. This used to be a function defined INSIDE App()
// and called as <TopDashboard/>. Because a brand new function (a brand
// new component type, as far as React is concerned) was created on every
// render, React unmounted and remounted the whole subtree each time,
// which reset TeamLogo's imgError state and cut short the stat-change
// highlight transitions. It's now a module-level component that receives
// props instead of reading from closure.
    ...
    <div className="w-full max-w-5xl mx-auto flex flex-col gap-3 mb-4 z-10 relative">
      <div className="glass-card rounded-2xl p-4 px-6 relative flex justify-between items-center">
        ...
        <div className="flex gap-4 items-center">
          {!isJunior && (
            <button onClick={onOpenShop} className="bg-white hover:bg-slate-50 border-2 border-slate-200 rounded-xl px-5 py-3 text-sm font-bold shadow-sm transition-all flex items-center gap-2 font-sans">
...
function App() {
  ...
  // Holds a pending playoff run while an in-season minigame/event plays out,
  // so making the playoffs no longer skips the mid-season drama.
  ...
  const advanceToOffseason = () => {
    ...
    if (isEuroLeague) {
      // BUG FIX #6: Europe used to be a one-way trip that, the instant the
      // contract ran out, dumped the player back into a full NHL
      // free-agency screen as if nothing had happened. Europe now renews
      // its own contract instead of routing through NHL free agency.
  ...
  // BUG FIX #2: handleTrain used to build `updatedPlayer` with the
  // training effect already applied, then hand BOTH that player and the
  // same effect to simulateSeason, which added the effect a second time.
  // Now the raw (pre-training) player and the effect are passed through
  // exactly once, and gameHelpers.simulateSeason is the single place that
  // applies it.
  const handleTrain = (t) => {
  ...
  // BUG FIX #10: this used to read the module-scoped `currentYear`
  // (derived from state that had just been updated in the same tick), so
  // the Olympics year check was reading last season's value. The correct
  // "next" year is now computed once in handleTrain and threaded through
  // as an argument.
  const runPostSeasonFlow = (pAge, pOvr, currentLg, currentTeam, madePlayoffs, nextYear, standings) => {
    // International tournaments take priority. They route to the playoffs
    // themselves (via seasonRecap.standings in handleContinueEvent).
    ...
    // Otherwise, ~65% of seasons now serve up an in-season moment (a minigame
    // or a locker-room event) BEFORE any playoff run. If the player made the
    // playoffs, we stash that run in pendingPlayoffs and pick it back up once
    // the moment resolves, so a good team no longer skips the drama entirely.
    if (Math.random() < 0.65) {
  ...
  // Picks a random scenario from the (much larger) position-appropriate pool.
  // The context is now passed in explicitly. Previously the Memorial Cup
  // called setMinigameContext('memcup') and then triggerMinigame() immediately
  // overwrote it back to 'season', so Memorial Cup matches never actually ran
  // their own win/loss logic. Passing context here fixes that.
  ...
  const handleMinigameChoice = (successChance, successMsg, failMsg, reward) => {
    ...
    // Regular-season context: apply the chosen archetype's variable payout.
    // OVR bumps route through applyOvrDelta so they survive the next season's
    // recompute, same as event choices.
  ...
  const handleEventChoice = (choice) => {
    ...
    // BUG FIX #3: see applyOvrDelta in gameHelpers.js.
    setPlayer(p => {
  ...
  // BUG FIX #4/#5: checkPlayoffs now takes the season's actual standing
  // and builds the win/loss deck (and playoff-spot count) from the
  // league's real size via gameHelpers.generatePlayoffDeck, instead of a
  // fixed 12-win/4-loss deck and a hardcoded random 1-16 opponent pool.
  ...
  // BUG FIX #8: this used to spread `player` shallowly and then mutate
  // `newP.stats.earnings`, `newP.inventory`, `newP.buffs` in place — all
  // the same nested objects/arrays as the original state, which React
  // can't reliably detect as a change. Everything below is rebuilt with
  // fresh objects/arrays inside the setPlayer updater.
  const buyItem = (item) => {
    setPlayer(p => {
  ...
  const handleContinueEvent = () => {
    if (minigameContext === 'memcup') {
      ...
      // Regular in-season moment finished. If a playoff run was waiting
      // behind it, go there now; otherwise straight to the recap.
      setActiveEvent(null);
      if (pendingPlayoffs) {
  ...
  return (
    <div className="min-h-screen p-3 md:p-6 bg-slate-50 relative flex flex-col font-sans">
      ...
      <Dashboard player={player} tier={tier} statChanges={statChanges} lgKey={lgKey} isJunior={isJunior} isAHL={isAHL} onOpenShop={() => setIsShopOpen(true)} />
        ...
        {screen === 'minigame' && (() => {
          ...
          return (
            <div className={`glass-card p-12 rounded-3xl mt-2 border-t-4 ${accent.border} text-center relative overflow-hidden font-sans`}>
              ...
              {isMemCup && (
                <p className="relative z-10 inline-block bg-amber-100 text-amber-700 border border-amber-300 text-xs font-black px-4 py-1.5 rounded-full uppercase tracking-widest mb-4">
              ...
              <p className="text-xl text-slate-600 mb-12 max-w-2xl mx-auto relative z-10 leading-relaxed">{mg.desc}</p>
                ...
                {mg.choices.map((c, i) => {
                  ...
                  return (
                    <button
                      ...
                      className={`bg-white hover:bg-slate-50 border-2 border-slate-200 ${pill.hover} py-7 px-4 rounded-2xl font-bold text-2xl text-slate-900 transition-all hover:shadow-md cursor-pointer flex flex-col items-center sports-font`}
                    >
                      {c.label}
                      <span className={`text-xs font-black mt-3 px-3 py-1 rounded-full uppercase tracking-widest font-sans border ${pill.cls}`}>
                        {pill.label} · {pct}%
                      </span>
                      <span className="text-[10px] text-slate-400 font-bold mt-2 uppercase tracking-wider font-sans">{c.tag}</span>
                      {!isMemCup && rewardBits.length > 0 && (
                        <span className="text-[11px] text-emerald-600 font-bold mt-2 font-sans normal-case tracking-normal">{rewardBits.join(' · ')}</span>
                      )}
                    </button>
                  );
                })}
